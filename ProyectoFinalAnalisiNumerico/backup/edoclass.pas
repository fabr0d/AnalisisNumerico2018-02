...
type
  TMethodsDif = class
    ...
    //output
    table: array of TStringList;
    //Siempre contiene a Yn en la posicion 0. Puede contener otras columnas como
    //IYn, k1 to kn, m, etc. Las cabeceras siempre estan en table[i][0] 0<=i<=size-1.
    //Los valores numericos en cualquier celdas menos las de la cabecera
...
begin
  Xn := X0;
  Yn := Y0;
  //ShowMessage('X0, Y0: '+FloatToStr(X0)+' ; '+FloatToStr(Y0));
...
begin
  ...
  ShowMessage('evaluciaion en la func:'+FloatToStr(Result));
end;

procedure TMethodsDif.setExpression(expr: String);
begin
 ShowMessage('expresion: '+expr);
